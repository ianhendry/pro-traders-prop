...
class SiteAccountResourceIT {
    ...
    @Mock
    private SiteAccountRepository siteAccountRepositoryMock;

    @Mock
    private SiteAccountService siteAccountServiceMock;
    ...
    void getAllSiteAccountsWithEagerRelationshipsIsEnabled() throws Exception {
        when(siteAccountServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restSiteAccountMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(siteAccountServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllSiteAccountsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(siteAccountServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
    ...
    void getAllSiteAccountsByAddressDetailsIsEqualToSomething() throws Exception {
        ...
        if (TestUtil.findAll(em, BillingDetails.class).isEmpty()) {
            addressDetails = BillingDetailsResourceIT.createEntity(em);
            em.persist(addressDetails);
            em.flush();
        } else {
            addressDetails = TestUtil.findAll(em, BillingDetails.class).get(0);
        }
        em.persist(addressDetails);
        em.flush();
