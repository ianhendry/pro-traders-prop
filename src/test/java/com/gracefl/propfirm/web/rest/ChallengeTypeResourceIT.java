...
class ChallengeTypeResourceIT {
    ...
    @Mock
    private ChallengeTypeRepository challengeTypeRepositoryMock;

    @Mock
    private ChallengeTypeService challengeTypeServiceMock;
    ...
    void getAllChallengeTypesWithEagerRelationshipsIsEnabled() throws Exception {
        when(challengeTypeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restChallengeTypeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(challengeTypeServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllChallengeTypesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(challengeTypeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
