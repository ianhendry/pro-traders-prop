...
class Mt4AccountResourceIT {
    ...
    void createMt4Account() throws Exception {
        ...
        assertThat(testMt4Account.getAccountID()).isEqualTo(DEFAULT_ACCOUNT_ID);
    ...
    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDIsEqualToSomething() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID equals to DEFAULT_ACCOUNT_ID
        defaultMt4AccountShouldBeFound("accountID.equals=" + DEFAULT_ACCOUNT_ID);

        // Get all the mt4AccountList where accountID equals to UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldNotBeFound("accountID.equals=" + UPDATED_ACCOUNT_ID);
    }

    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDIsNotEqualToSomething() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID not equals to DEFAULT_ACCOUNT_ID
        defaultMt4AccountShouldNotBeFound("accountID.notEquals=" + DEFAULT_ACCOUNT_ID);

        // Get all the mt4AccountList where accountID not equals to UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldBeFound("accountID.notEquals=" + UPDATED_ACCOUNT_ID);
    }

    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDIsInShouldWork() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID in DEFAULT_ACCOUNT_ID or UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldBeFound("accountID.in=" + DEFAULT_ACCOUNT_ID + "," + UPDATED_ACCOUNT_ID);

        // Get all the mt4AccountList where accountID equals to UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldNotBeFound("accountID.in=" + UPDATED_ACCOUNT_ID);
    }

    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDIsNullOrNotNull() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID is not null
        defaultMt4AccountShouldBeFound("accountID.specified=true");

        // Get all the mt4AccountList where accountID is null
        defaultMt4AccountShouldNotBeFound("accountID.specified=false");
    }

    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDContainsSomething() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID contains DEFAULT_ACCOUNT_ID
        defaultMt4AccountShouldBeFound("accountID.contains=" + DEFAULT_ACCOUNT_ID);

        // Get all the mt4AccountList where accountID contains UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldNotBeFound("accountID.contains=" + UPDATED_ACCOUNT_ID);
    }

    @Test
    @Transactional
    void getAllMt4AccountsByAccountIDNotContainsSomething() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);

        // Get all the mt4AccountList where accountID does not contain DEFAULT_ACCOUNT_ID
        defaultMt4AccountShouldNotBeFound("accountID.doesNotContain=" + DEFAULT_ACCOUNT_ID);

        // Get all the mt4AccountList where accountID does not contain UPDATED_ACCOUNT_ID
        defaultMt4AccountShouldBeFound("accountID.doesNotContain=" + UPDATED_ACCOUNT_ID);
    }
    ...
    void getAllMt4AccountsByMt4TradeIsEqualToSomething() throws Exception {
        ...
        mt4AccountRepository.saveAndFlush(mt4Account);
        Mt4Trade mt4Trade;
        if (TestUtil.findAll(em, Mt4Trade.class).isEmpty()) {
            mt4Trade = Mt4TradeResourceIT.createEntity(em);
            em.persist(mt4Trade);
            em.flush();
        } else {
            mt4Trade = TestUtil.findAll(em, Mt4Trade.class).get(0);
        }
        em.persist(mt4Trade);
        em.flush();
    ...
    void getAllMt4AccountsByAccountDataTimeSeriesIsEqualToSomething() throws Exception {
        // Initialize the database
        mt4AccountRepository.saveAndFlush(mt4Account);
        AccountDataTimeSeries accountDataTimeSeries;
        if (TestUtil.findAll(em, AccountDataTimeSeries.class).isEmpty()) {
            accountDataTimeSeries = AccountDataTimeSeriesResourceIT.createEntity(em);
            em.persist(accountDataTimeSeries);
            em.flush();
        } else {
            accountDataTimeSeries = TestUtil.findAll(em, AccountDataTimeSeries.class).get(0);
        }
        em.persist(accountDataTimeSeries);
        em.flush();
    ...
    void putNewMt4Account() throws Exception {
        ...
        Mt4Account testMt4Account = mt4AccountList.get(mt4AccountList.size() - 1);
        assertThat(testMt4Account.getAccountID()).isEqualTo(UPDATED_ACCOUNT_ID);
