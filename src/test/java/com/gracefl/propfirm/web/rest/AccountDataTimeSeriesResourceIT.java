...
class AccountDataTimeSeriesResourceIT {
    ...
    @Mock
    private AccountDataTimeSeriesRepository accountDataTimeSeriesRepositoryMock;

    @Mock
    private AccountDataTimeSeriesService accountDataTimeSeriesServiceMock;
    ...
    void getAllAccountDataTimeSeriesWithEagerRelationshipsIsEnabled() throws Exception {
        when(accountDataTimeSeriesServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restAccountDataTimeSeriesMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(accountDataTimeSeriesServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllAccountDataTimeSeriesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(accountDataTimeSeriesServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
