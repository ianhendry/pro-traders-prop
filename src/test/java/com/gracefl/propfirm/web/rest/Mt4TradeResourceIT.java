...
class Mt4TradeResourceIT {
    ...
    @Mock
    private Mt4TradeRepository mt4TradeRepositoryMock;

    @Mock
    private Mt4TradeService mt4TradeServiceMock;
    ...
    void getAllMt4TradesWithEagerRelationshipsIsEnabled() throws Exception {
        when(mt4TradeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restMt4TradeMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(mt4TradeServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllMt4TradesWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(mt4TradeServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));
